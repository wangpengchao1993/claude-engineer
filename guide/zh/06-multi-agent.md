...
AI Agent 的编排从简单到复杂经历了三个阶段：

### Level 1: 单 Agent（Claude Code 默认模式）

用户直接和一个 Agent 对话。Agent 可以读写文件、运行命令，但所有决策由同一个上下文完成。

适合：日常编码、小功能、bug 修复。

### Level 2: 多 Agent 扇出/扇入（本章重点）

一个编排器将任务分发给多个专门化的 Agent，各自并行处理后汇总结果。

适合：多文件重构、多角度 Review、大规模测试生成。

### Level 3: 自编排 Agent（前沿方向）

Agent 自己决定需要什么子 Agent、怎么分工、何时汇总。例如 Hermes Agent 框架的自进化模式。
...
| 场景 | 推荐 | 原因 |
|------|------|------|
| 改一个函数 | Level 1 | 单 Agent 足够，启动快 |
| 重构 10 个文件 | Level 2 | 并行加速，各 Agent 专注一个方面 |
| 从零构建一个系统 | Level 2-3 | 需要规划→设计→实现→测试的完整链路 |
