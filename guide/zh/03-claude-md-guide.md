...
不同的上下文窗口大小，CLAUDE.md 的写法应该不同。

### 200K 窗口（默认）

上下文宝贵，CLAUDE.md 要精简：
- 控制在 **200-500 行**
- 只写 AI 不看代码就不知道的信息（约束、规范、命令）
- 不写 AI 能从代码推断的信息（技术栈、文件结构）
- 主动用 `/compact` —— 当 Claude 的回答开始变差或变慢时就该压缩

### 1M 窗口（Bedrock/Vertex）

上下文充裕，CLAUDE.md 可以更丰富：
- 可以扩展到 **1000-3000 行**
- 可以包含架构决策的原因（"为什么选了 X 而不是 Y"）
- 可以包含代码示例（"好的代码长这样"）
- `/compact` 频率大幅降低
...
| Claude 开始重复之前说过的话 | 是 |
| 回答质量明显下降 | 是 |
| `/cost` 显示 token 接近上限 | 是 |
| 刚做完一个完整功能，要开始下一个 | 是（清理上下文） |
